...
RNA_merged_data <- RunUMAP(RNA_merged_data, dims = 1:50)
DimPlot(RNA_merged_data, reduction = "umap", label = TRUE) + ggtitle("Cell Type Clusters")
```


```{r}
#Manual Annotation 
#Now let's extract the top marker genes, and see which ones correspond with each cluster. This can be done 
...
mouse_brain <- read.csv("biorxiv_mouse_brain.csv")
...
Dim_plot <- DimPlot(RNA_merged_data, reduction = "umap", label = FALSE, pt.size = 0.5) + ggtitle("Clusters 
by cell type", aes(color = 1)) + theme_bw() + theme(axis.text.x = element_text(size = 15, colour = "black"), 
axis.text.y = element_text(size = 15, colour = "black"), plot.title = element_text(size = 17, face = 
"bold"))

Dim_plot <- Dim_plot + theme(text = element_text(size = 14), legend.text = element_text(size = 15, face = 
"bold"))
